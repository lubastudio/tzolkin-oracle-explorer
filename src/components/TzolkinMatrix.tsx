...
const TzolkinMatrix: React.FC<TzolkinMatrixProps> = ({ selectedKin, onKinSelect }) => {
    ...
    for (let toneIndex = 0; toneIndex < 13; toneIndex++) {
      ...
      for (let sealIndex = 0; sealIndex < 20; sealIndex++) {
        ...
        cells.push(
          <div 
            key={`kin-${kin}`}
            className={`tzolkin-cell w-6 h-6 md:w-6 md:h-6 lg:w-8 lg:h-8 border border-black text-[10px] md:text-xs flex items-center justify-center ${getKinColorClass(kin)} ${
  ...
  return (
    <div className="px-0 py-4 md:px-4">
      <h2 className="text-base mb-4 text-black text-center">Matriz Tzolkin</h2>
